...
2. Model 2: including the basic predictors and control variables. Other potential factors that were not specifically hypothesised but could influence ratings were added as control variables in model 2. This included `startYear`, `runtimeMinutes` and `numVotes`. Upon initial inspection, the distribution of `runtimeMinutes` and `numVotes` was not normal, whereby these have been log transformed and entered into the model as `log_runtimeMinutes` and `log_numVotes`.
