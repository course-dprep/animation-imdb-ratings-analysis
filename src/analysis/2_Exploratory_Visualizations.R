...
ggsave("temp/output/hist_average_rating.png", p1, width = 7, height = 5)
...
p2 <- ggplot(movies, aes(x=startYear, fill = animation_dummy)) + 
  ...
  theme_classic()
ggsave("temp/output/hist_movies_per_year.png", p2, width = 7, height = 5)
...
write_csv(ratio, "temp/output/ratio_non_vs_animation.csv")
...
ggsave("temp/output/line_avg_rating_all.png", p3, width = 7, height = 5)
...
p4 <- ggplot(graph_splitmovies, aes(x=startYear, y=meanRating, 
  ...
  theme_minimal()
ggsave("temp/output/line_avg_rating_by_type.png", p4, width = 7, height = 5)
...
p5 <- ggplot(movies, aes(x = startYear, y = averageRating, colour = animation_dummy)) +
  ...
  labs (x = "Year", y = "Average IMDb Rating",
        ...
        colour = "Type of Movie") +
  theme_classic()
ggsave("temp/output/smooth_avg_rating_by_type.png", p5, width = 7, height = 5)
...
ggsave("temp/output/boxplot_runtime.png", p6, width = 7, height = 5)
