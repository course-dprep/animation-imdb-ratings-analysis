...
The data sets used in this project are title.basics.tsv.gz and title.ratings.tsv.gz. Title.basics contains basic information about all titles on IMDb. Think about the name, release year, duration of the movie, and genre(s). Title.ratings contains information about the ratings per title. The average rating and the number of votes are provided in this data set.

Initially, the dataset holds data on different types of mediums, including movies, series, tv shows, shorts, etc. In this project, a subset of the data set is used, consisting only of movies, as the entire data set is too large and caused problems on some older laptops during data download.
...
As described in data size, the tconst variable is the join key and unique identifier used to merge the two initial datasets. The unit of analysis in the data sets, is one movie per row. Neither dataset holds temporal information on date.
...
- endYear, is mostly relevant to series, and contains a lot of NA's 
- runtimeMinutes, shows how long the movie is 
- genres, contains a comma-seperated list, for movies that fall under multiple genre's
...
First, the dataset movies_selected was checked for missing values. A visual inspection of genres revealed additional missing values encoded by IMDb with other values, instead of NA.  These were therefore recoded to NA. Hereafter, a second check confirmed that missing values remained in the variables startYear, runtimeMinutes, genres, averageRating, and numVotes.
